def call(repoName, targetName, releaseManifest = 'release.txt') {
  def agent_env = env.BRANCH_NAME == 'master' ? 'agent' : 'agent-staging';
  withCredentials([
        ...
        string(credentialsId: "${repoName}-targets-key-pw", variable: "NOTARY_TARGETS_PASSPHRASE")
          ])
          {
            ...
            sh "curl --user ${NEXUS_UPLOAD_USER}:${NEXUS_UPLOAD_PASS} --upload-file target.tgz https://nexus.gridpl.us/repository/${agent_env}/${repoName}/${targetName}"
